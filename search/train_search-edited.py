...
def main(genome, epochs, search_space='micro',
    ...
    # adversarial_examples = pickle.load(open("search/adversarial_dataset.pkl", "rb"))

    # valid_queue_adversarial = torch.utils.data.DataLoader(
    #     adversarial_examples, batch_size=batch_size,
    #     # sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[split:num_train]),
    #     pin_memory=True, num_workers=4)
    #
    # something, adversarial_acc = adv_infer(valid_queue_adversarial, model, criterion)
    # logging.info('adversarial_acc %f', adversarial_acc)
    ...
    with open(os.path.join(save_pth, 'log.txt'), "w") as file:
        ...
        # file.write("adversarial_acc = {}\n".format(adversarial_acc))
...
def adv_infer(valid_queue, net, criterion):
    net.eval()
    test_loss = 0
    correct = 0
    total = 0
