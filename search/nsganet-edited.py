...
def nsganet(
    ...
    #     f = open("search-Robustness-macro-20200718-044336/arch_" + str(idx) + "/log.txt", "r")
    #     contents = f.read()
    #     genome = (contents.split('\n')[0]).split('=')[1]
    #     genome = eval(genome)
    #
    #     temp_array = np.array([])
    #
    #     for g in genome:
    #         temp_g = np.array(g)
    #         temp_array = np.concatenate((temp_array, temp_g), axis=None)
    #
    #     genome = np.array(temp_array)
    #     print(genome)
    #     array.append(genome)
    #
    ...
    # cwd = os.getcwd()
    # print(cwd)
    # custom_pop = pickle.load(open("search-Robustness-macro-20200710-041436\pops\gen_14.pckl", "rb"))
