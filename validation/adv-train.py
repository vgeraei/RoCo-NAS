...
def main():
    ...
    parser.add_argument('--save', type=str, default='NAS33-120ep', help='experiment name')
    ...
    parser.add_argument('--arch', type=str, default='nas2', help='which architecture to use')
...
def train(train_queue, net, criterion, optimizer):
    ...
    total = 0
    global args
    pgd_attack = PGD(net.float(), eps=8 / 255, steps=20)  # Training
    # pgd_attack = PGD(net.float(), eps=8 / 255, steps=40) # Evaluation
    for step, (inputs, targets) in enumerate(train_queue):
        inputs, targets = inputs.to(device), targets.to(device)
        inputs = inputs.float()
        inputs = pgd_attack(inputs, targets).cuda()
        optimizer.zero_grad()
        outputs = net(inputs)
